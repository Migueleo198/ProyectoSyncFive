...
class CarnetModel
{
    ...
    {
        return $this->db
            ->query("SELECT *, categoria AS grupo FROM Carnet ORDER BY id_carnet ASC")
            ->fetchAll();
    }
    ...
    public function update(string $id_carnet, array $data): int
    {
        ...
        ->bind(':categoria', $data['grupo'] ?? null)
...
{
    return $this->db
        ->query("SELECT *, categoria AS grupo FROM Carnet WHERE id_carnet = :id_carnet")
        ->bind(':id_carnet', $id_carnet)
        ->fetch();
}
  /**
