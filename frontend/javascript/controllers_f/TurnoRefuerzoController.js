import TurnoRefuerzoApi from '../api_f/TurnoRefuerzoApi.js';

let turnosRefuerzo = []; // variable global para almacenar turnos de refuerzo

document.addEventListener('DOMContentLoaded', () => {
  cargarTurnoRefuerzo();
/*   cargarPersona(); // Cargar personas para mostrar su nombre en la tabla de turnos de refuerzo
  bindCrearTurnoRefuerzo();
  bindEliminarTurnoRefuerzo(); */
  // cargarSelectVehiculos();
});

// ================================
// CARGAR Turnos de Refuerzo
// ================================
async function cargarTurnoRefuerzo() {
  try {
    const response = await TurnoRefuerzoApi.getAll();
    turnosRefuerzo = response.data; // guardamos globalmente
    renderTablaTurnosRefuerzo(turnosRefuerzo);
  } catch (e) {
...
/* async function cargarPersona() {
        ...
        const personas = await TurnoRefuerzoApi.getPersona();
...
// RENDER TABLA
// ================================
function renderTablaTurnosRefuerzo(turnosRefuerzo) {
  const tbody = document.querySelector('#tabla tbody');
  tbody.innerHTML = '';

  turnosRefuerzo.forEach(t => {
    
    const tr = document.createElement('tr');

    /*
      const fechaFormateada = formatearFecha(t.fecha);
      const fIniFormateada = formatearFecha(t.f_Ini);
      const fFinFormateada = formatearFecha(t.f_Fin);
      
      <td class="d-none d-md-table-cell">${t.id_turnoRefuerzo}</td>
      <td>${t.nombreFuncionario}</td>
      <td class="d-none d-md-table-cell">${fechaFormateada}</td>
      <td>${fIniFormateada}</td>
...
// ================================
// CREAR / INSERTAR Turno de Refuerzo
// ================================
function bindCrearTurnoRefuerzo() {
  const form = document.getElementById('formTurnoRefuerzo');
    ...
    const data = {
      id_turnoRefuerzo: f.get('id_turnoRefuerzo'),
      f_Ini: f.get('f_Ini'),
      f_Fin: f.get('f_Fin')
    };

    try {
      await TurnoRefuerzoApi.create(data); // ← INSERT al backend
      await cargarTurnosRefuerzo();        // ← refrescar tabla
      form.reset();
      alert('Turno de refuerzo creado correctamente');
    } catch (err) {
