...
function bindModalEquipo() {
  ...
  // Al abrirse el modal, detectar desde dónde se abre y renderizar
  document.getElementById('modalInsertarEquipo')
    .addEventListener('show.bs.modal', (event) => {
      const trigger = event.relatedTarget;

      // ✅ Comprobación segura de trigger (puede ser null si se abre programáticamente)
      modalEquipoDesdeInsertar = !!(trigger && trigger.closest && trigger.closest('#formIncidencia'));

      // Solo resetear vehículos si viene del formulario de insertar
  ...
  // Mantener modal-open en el body al mostrarse (para scroll correcto)
  document.getElementById('modalInsertarEquipo')
  ...
  document.getElementById('btnAnadirVehiculo').addEventListener('click', () => {
    ...
    if (vehiculosEnModal.find(v => v.matricula === matricula)) {
      mostrarError('Este vehículo ya está añadido');
      return;
    }
  ...
  document.getElementById('btnGuardarEquipo').addEventListener('click', () => {
    ...
    // Asignar fechas solo a los vehículos nuevos
    ...
    // ✅ Actualizar resumen en el modal Editar
    const resumenEditar = document.getElementById('resumenVehiculosEditar');
    if (resumenEditar) {
      resumenEditar.value = resumenTexto;
    }

    // ✅ Actualizar resumen en el formulario Insertar
    const resumenInsertar = document.getElementById('resumenVehiculosInsertar');
    if (resumenInsertar) {
      resumenInsertar.textContent = resumenTexto;
    }
    ...
    // Reabrir modal Editar solo si se llegó desde allí
    if (!modalEquipoDesdeInsertar) {
...
function renderVehiculosModal() {
  ...
  vehiculosEnModal.forEach((vehiculo, idx) => {
    ...
    bloque.innerHTML = `
      <div class="d-flex justify-content-between align-items-center mb-2">
        ...
        <button type="button"
                class="btn btn-sm btn-outline-secondary btn-quitar-vehiculo"
                data-idx="${idx}">
          Quitar
        </button>
      </div>
      ...
      <div class="row mb-2 align-items-end">
        ...
        <div class="col-md-4">
          <button type="button"
                  class="btn btn-sm btn-outline-primary w-100 btn-añadir-persona"
                  data-idx="${idx}">
            + Persona
          </button>
        </div>
          ...
          ${vehiculo.personas.length === 0
            ...
            : vehiculo.personas.map((p, pi) => `
                <tr>
                  ...
                  <td>
                    <button type="button"
                            class="btn btn-sm btn-outline-danger btn-quitar-persona"
                            data-idx="${idx}"
                            data-pi="${pi}">✕</button>
                  </td>
                </tr>
  ...
  contenedor.querySelectorAll('.btn-quitar-persona').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      vehiculosEnModal[idx].personas.splice(pi, 1);
      renderVehiculosModal();
    });
  });
}
...
function bindCrearEmergencia() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      // ✅ Resetear resumen de vehículos en el formulario insertar
      const resumenInsertar = document.getElementById('resumenVehiculosInsertar');
      if (resumenInsertar) {
        resumenInsertar.textContent = 'Sin vehículos asignados';
      }
...
document.addEventListener('click', async function (e) {
  ...
  try {
    ...
    // Resetear vehículos y cargar los de esta emergencia
    vehiculosEnModal = [];
  ...
  const vehiculosBase = respVehiculos.data || [];
    ...
    // ✅ Cargar personal de cada vehículo en paralelo
      ...
      vehiculosBase.map(async (v) => {
        ...
        try {
          ...
          personas = (respPersonal.data || []).map(p => ({
            ...
            nombre:     p.nombre ?? p.id_bombero,
          }));
        } catch { /* si falla, personas queda vacío */ }
    ...
    // Guardamos snapshot de los originales para calcular diff al guardar
    ...
    form.innerHTML = `
      ...
      <div class="row mb-3">
        ...
        <div class="col-lg-8">
          ...
          <div class="input-group">
            <!-- ✅ Se actualiza al volver del modal de equipo -->
            <input type="text"
                   class="form-control"
      ...
      <div class="mb-3">
        ...
        <textarea class="form-control"
                  name="descripcion"
                  rows="4">${emergencia.descripcion || ''}</textarea>
      </div>
...
document.addEventListener('click', async function (e) {
  ...
  filas.forEach(f => {
    html += `
      <tr>
        <th class="table-secondary w-40">${f.label}</th>
        <td>${f.valor}</td>
      </tr>`;
  });
  html += '</table>';
          ...
          <div class="border rounded p-3 mb-3">
            ...
            <div class="row text-muted small mb-2">
              <div class="col-md-4">
                ...
                ${vehiculo.f_salida ? formatearFechaHora(vehiculo.f_salida) : '—'}
              </div>
              <div class="col-md-4">
                <strong>Llegada:</strong>
                ${vehiculo.f_llegada ? formatearFechaHora(vehiculo.f_llegada) : '—'}
              </div>
              <div class="col-md-4">
                <strong>Regreso:</strong>
                ${vehiculo.f_regreso ? formatearFechaHora(vehiculo.f_regreso) : '—'}
              </div>
            </div>`;
          ...
          if (personal.length === 0) {
            ...
            html += `
              <table class="table table-sm table-bordered mb-0 text-center">
                <thead class="table-secondary">
                  <tr><th>ID Bombero</th></tr>
                </thead>
                <tbody>
                  ${personal.map(p => `
                    <tr><td>${p.id_bombero ?? '—'}</td></tr>
                  `).join('')}
                </tbody>
              </table>`;
          }
